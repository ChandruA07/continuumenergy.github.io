...
function submit33Progress(e){
  e.preventDefault();
  const feeder = document.getElementById('p33-feeder')?.value || '';
  const act    = document.getElementById('p33-act')?.value    || '';
  const qty    = +(document.getElementById('p33-qty')?.value  || 0);
  const mp     = +(document.getElementById('p33-mp')?.value   || 0);
  const row    = +(document.getElementById('p33-row')?.value  || 0);
  const rem    = document.getElementById('p33-rem')?.value    || '';

  // Write to Firebase so all devices see the entry
  if(typeof dataApi !== 'undefined' && dataApi.addDailyProgress){
    dataApi.addDailyProgress({
      module:   'BOP-33kV',
      activity: feeder + ' › ' + act,
      qty,
      unit:     'poles',
      remarks:  (row ? 'ROW issues: '+row+'. ' : '') + rem
    }).catch(err => console.warn('[submit33Progress] Firebase write error:', err));
  }
...
function submit66Progress(e){
  e.preventDefault();
  const act  = document.getElementById('p66-act')?.value   || '';
  const done = +(document.getElementById('p66-done')?.value || 0);
  const mp   = +(document.getElementById('p66-mp')?.value   || 0);
  const rem  = document.getElementById('p66-rem')?.value    || '';

  // Write progress to Firebase so all devices see it
  if(typeof dataApi !== 'undefined' && dataApi.addDailyProgress){
    dataApi.addDailyProgress({
      module:   'BOP-66kV',
      activity: act,
      ...
      unit:     'towers',
      remarks:  rem
    }).catch(err => console.warn('[submit66Progress] Firebase write error:', err));
  }
  DB.mp.bop+=mp;updateKpiMp();
  cov('pov');
...
function submitPSSGSSProg(e,sec){
  ...
  // Write to Firebase — listener fans the change to all devices
  const apiFn = sec==='pss' ? dataApi.updatePssAct : dataApi.updateGssAct;
  if(typeof dataApi !== 'undefined' && apiFn){
    apiFn(act, { done: cum, wip }).catch(err => console.warn('[submitPSSGSS] Firebase write error:', err));
  }

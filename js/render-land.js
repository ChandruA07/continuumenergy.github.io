...
function removeWtgLand(id){
  if(!confirm(`Remove ${id}?`))return;
  const i=DB.wtgLand.locs.findIndex(x=>x.id===id);
  if(i>=0)DB.wtgLand.locs.splice(i,1);
  rndrWtgLand();
  // Note: this is local-only display removal; the master list lives on Firebase
  // and is admin-managed via direct console access for now.
}

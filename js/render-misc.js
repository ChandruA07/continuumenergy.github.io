...
function subPOD(e,t){
  ...
  // Write to Firebase through dataApi — Firebase listener fans the change back
  // to appState on ALL devices (laptop, mobile, etc.)
  dataApi.addPod({
    module:     t,
    activity:   actLabel,
    qty,
    ...
    console.error('[subPOD] Firebase write failed:', err);
    showToast('❌ Submit failed: ' + (err.message||err), 'er');
  });
}
...
function saveObsEdit(idx){
  const o=HSE_DB.observations[idx];if(!o)return;
  const newObs=document.getElementById('eo-obs').value;
  const newStatus=document.getElementById('eo-status').value;
  const newClosedBy=document.getElementById('eo-closedby').value;

  // Write to Firebase first — listener updates HSE_DB and re-renders on all devices
  if(o._id && typeof dataApi !== 'undefined' && dataApi.updateHseObservation){
    dataApi.updateHseObservation(o._id, {
      obs:      newObs,
      status:   newStatus,
      closedBy: newClosedBy
    }).catch(err => console.warn('[saveObsEdit] Firebase write error:', err));
  }
  // Optimistic local update so the current device doesn't wait for the round-trip
  o.obs=newObs;
  o.status=newStatus;
  o.closedBy=newClosedBy;
  ...
  cov('pov');rndrSafety();showToast('Observation updated!','ok');
}
...
function saveEmpEdit(code){
  const e=HSE_DB.employees.find(x=>x.code===code);if(!e)return;
  const newName=document.getElementById('ee-name').value;
  const newScore=+document.getElementById('ee-score').value||e.score;

  if(e._id && typeof dataApi !== 'undefined' && dataApi.updateHseEmployee){
    dataApi.updateHseEmployee(e._id, { name: newName, score: newScore })
      .catch(err => console.warn('[saveEmpEdit] Firebase write error:', err));
  }
  // Optimistic local update
  e.name=newName;
  e.score=newScore;
  cov('pov');rndrSafety();showToast('Employee updated!','ok');
}
function removeEmployee(code){
  if(!confirm('Remove employee '+code+'?'))return;
  const e=HSE_DB.employees.find(x=>x.code===code);
  if(e && e._id && typeof dataApi !== 'undefined' && dataApi.deleteHseEmployee){
    dataApi.deleteHseEmployee(e._id)
      .catch(err => console.warn('[removeEmployee] Firebase delete error:', err));
  }
  // Optimistic local remove
  HSE_DB.employees=HSE_DB.employees.filter(e=>e.code!==code);
  rndrSafety();showToast('Employee removed','wn');
}
